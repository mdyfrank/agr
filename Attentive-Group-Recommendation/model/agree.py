...
class AGREE(nn.Module):
    def __init__(self, num_users, num_items, num_groups, embedding_dim, group_member_dict, group_member_dict_neg, drop_ratio):
        ...
        self.group_member_dict_neg = group_member_dict_neg
    ...
    def forward(self, group_inputs, user_inputs, item_inputs):
        ...
        # train user
        elif (group_inputs is None) and (user_inputs is not None):
            out = self.usr_forward(user_inputs, item_inputs)
        elif (group_inputs is not None) and (user_inputs is not None):
            out = self.usr_group_forward(group_inputs, user_inputs, item_inputs)
        return out
    ...
    # user_group forward - w/o attention
    def usr_group_forward_simple(self, group_inputs, user_inputs):
        group_embeds = self.groupembeds(group_inputs)
        user_embeds = self.userembeds(user_inputs)

        element_embeds = torch.mul(group_embeds, user_embeds)
        new_embeds = torch.cat((element_embeds, group_embeds, user_embeds), dim=1)
        y = torch.sigmoid(self.predictlayer(new_embeds))
        return y

    def usr_group_forward(self, group_inputs, user_inputs, item_inputs):
        group_embeds = torch.Tensor()
        user_embeds = self.userembeds(user_inputs)
        for i, j in zip(group_inputs, item_inputs):
            members = self.group_member_dict[i.item()]
